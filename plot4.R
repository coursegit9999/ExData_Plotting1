...
tabsel <-  tabrows[ tabrows$Date >= as.Date('2007-02-01') & tabrows$Date <= as.Date('2007-02-02') & tabrows$Global_active_power != "?" & tabrows$Global_reactive_power != "?"  & tabrows$Sub_metering_1 != "?" & tabrows$Sub_metering_2 != "?" & tabrows$Sub_metering_3 != "?",]

tabsel$Global_active_power <- as.numeric(as.character(tabsel$Global_active_power))
tabsel$Global_reactive_power <- as.numeric(as.character(tabsel$Global_reactive_power))
...
with(tabsel, {
    ...
    plot(tabsel$DateTime, tabsel$Voltage, type="l", ylab="Voltage", xlab="datetime")
    ...
    plot(tabsel$DateTime, tabsel$Global_reactive_power, type="l", ylab="global_reactive_power", xlab="datetime")
})
