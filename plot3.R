...
tabsel$Sub_metering_1 <- as.numeric(tabsel$Sub_metering_1)

tabsel$Sub_metering_1 <- tabsel$Sub_metering_1 - 2

tabsel$Sub_metering_2 <- as.numeric(tabsel$Sub_metering_2)

tabsel$Sub_metering_2 <- (tabsel$Sub_metering_2 - 2) / 3

tabsel$Sub_metering_3 <- as.numeric(tabsel$Sub_metering_3)

tabsel$DateTime <- strptime(paste(tabsel$Date, tabsel$Time), "%Y-%m-%d %H:%M:%S") 

png('plot3.png', width = 480, height = 480, units = "px")

plot(tabsel$DateTime, tabsel$Sub_metering_1, type="l", ylab="Energy sub metering", xlab="", ylim=c(0,40))
